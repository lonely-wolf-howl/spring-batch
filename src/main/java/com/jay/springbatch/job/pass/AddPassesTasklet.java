...
public class AddPassesTasklet implements Tasklet {
  ...
  public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) {
    ...
    final List<BulkPassEntity> bulkPassEntities = bulkPassRepository.findByStatusAndStartedAtAfter(BulkPassStatus.READY, startedAt);
    ...
    for (BulkPassEntity bulkPassEntity : bulkPassEntities) {
      ...
      createdCount += addPasses(bulkPassEntity, userIds);

      bulkPassEntity.setStatus(BulkPassStatus.COMPLETED);
    }
    log.info("AddPassesTasklet - execute: issued {} passes, startedAt={}", createdCount, startedAt);
